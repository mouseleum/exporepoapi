...
9. **HubSpot enrichment on /library/companies** — Private App–authed integration that pulls HubSpot Companies + their associated Deals/Engagements/Owners, matches against `companies.website` by registrable domain, and writes per-company signals (met_by_me / met_by_team / in_pipeline + last engagement + latest open deal) to a new `company_hubspot_signals` table. UI adds a "Connected as <email> · Last synced … · [Sync now]" strip, three badges per row (💼 You / 👥 Team / 📈 Pipeline), and four filter chips. On-demand sync only (no cron). Files: `db/migrations/0009_hubspot_oauth_and_signals.sql`, `lib/hubspot/{auth,client,domain,sync}.ts`, `app/api/hubspot/{status,sync}/route.ts`. Env: `HUBSPOT_ACCESS_TOKEN` (Private App bearer from Settings → Integrations → Private Apps), optional `HUBSPOT_OWNER_EMAIL` (for met-by-you distinction). ✅ shipped 2026-06-08; switched OAuth → Private App on 2026-06-09 because creating a HubSpot Developer account was friction without a multi-portal use case. Apply the 0009 migration manually in Supabase before first sync.
